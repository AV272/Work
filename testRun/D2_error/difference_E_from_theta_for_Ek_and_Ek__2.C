...
void difference_E_from_theta_for_Ek_and_Ek__2(){
    ...
    double E1_k_arr[4] = {0.05, 0.02, 0.03, 0.05};

    int n3 = 180, n3_2 = 360, n4=180;

    double E_d_arr[n3_2], E_p_arr[n3], theta_p_arr[n3], dE_p_arr[n3], dTheta_p_arr[n3], dE_T_arr[n3],\
           dTheta_T_arr[n3], E_T_arr[n3], theta_T_arr[n3], E_25_p_arr[n3], E_25_T_arr[n3], E_50_p_arr[n3], E_50_T_arr[n3];
            ...
            for (int i7=0; i7<2; i7++){
                ...
                if(i7==1){
                    for (int i5 = 0; i5 < n3; i5++){
                        E_25_p_arr[i5] = E_p_arr[i5];
                        E_25_T_arr[i5] = E_T_arr[i5];
                    }           
                }else{
                    for (int i5 = 0; i5 < n3; i5++){
                        E_50_p_arr[i5] = E_p_arr[i5];
                        E_50_T_arr[i5] = E_T_arr[i5];
                    }
                }
            }

            for(int i6=0; i6<n3; i6++){
                dE_p_arr[i6] = E_50_p_arr[i6] - E_25_p_arr[i6];
                dE_T_arr[i6] = E_50_T_arr[i6] - E_25_T_arr[i6];
            }

            TGraph* gr = new TGraph(n3, theta_p_arr, dE_p_arr);
            gr->GetXaxis()->SetTitle("#theta(degres)");
            gr->GetYaxis()->SetTitle("dE_{kinetic}(Mev)");
            if(i4==0){
                ...
                gr->SetLineColor(1);
                g_name.Form("proton_%f_%f",E1_k*2, E1_k);
            }else{
                // gr->SetMarkerColor(ii+33);
                // gr->SetMarkerStyle(ii+39);
                // gr->SetLineColor(ii+33);
                gr->SetMarkerColor(2);
                gr->SetMarkerStyle(21);
                gr->SetLineColor(2);
                g_name.Form("neutron_%f_%f",E1_k*2, E1_k);

            }
            gr->SetTitle(g_name);
            mg->Add(gr);     
                    
            TGraph* gr2 = new TGraph(n3, theta_T_arr, dE_T_arr);
            gr2->GetXaxis()->SetTitle("#theta(degres)");
            gr2->GetYaxis()->SetTitle("dE_{kinetic}(Mev)");
            
            if(i4==0){
                g_name.Form("Triton_%f_%f",E1_k*2, E1_k);
                // gr2->SetMarkerColor(ii+5);
                // gr2->SetMarkerStyle(ii+24);
                // gr2->SetLineColor(ii+5);
                gr2->SetMarkerColor(3);
                gr2->SetMarkerStyle(22);
                gr2->SetLineColor(3);
            }else{
                g_name.Form("Helium_%f_%f",E1_k*2, E1_k);
                // gr2->SetMarkerColor(ii+39);
                // gr2->SetMarkerStyle(ii+46);
                // gr2->SetLineColor(ii+39);
    ...
    //c1->cd(1);
    mg->SetTitle("Difference between E_{k} and E_{k}/2 for p, 3H and 3He from angle (#theta) in lab system");
    mg->GetXaxis()->SetTitle("#it{#theta} (degres)");
    mg->GetYaxis()->SetTitle("dE_{kinetic}(Mev)");
    gPad->Modified();
    mg->Draw("ap");
